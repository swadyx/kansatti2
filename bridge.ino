...
bool initCamera() {
    ...
    config.fb_count=1; config.grab_mode=CAMERA_GRAB_LATEST;
...
bool handleBridgeCommand(const Command &c) {
    ...
    memcpy(buf, c.cmd, sizeof(c.cmd)); buf[sizeof(c.cmd)] = '\0';
    String cmd = String(buf); cmd.trim();
    if (cmd == "photo")    { takePhoto(); return true; }
    if (cmd == "startcam") { continuousCapture = true;  return true; }
    if (cmd == "stopcam")  { continuousCapture = false; return true; }
    return false;
}
...
void loop() {
    ...
    if (continuousCapture && (millis()-lastCaptureTime >= captureInterval)) {
        takePhoto(); lastCaptureTime = millis();
    }
}