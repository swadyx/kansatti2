...
static void applyCommand(uint8_t cmd) {
    switch (cmd) {
        ...
        case 12:
            ...
            rollRate_prev  = pitchRate_prev = yawRate_prev = 0.0f;
            resetPIDState();
            // altHoldInit();           // <-- add this
            Serial.println("[FC] ARMED — ramping");
            break;
    break;
        case 13:
...
void loop() {
    ...
    if (fabs(rollEst) > 30 || fabs(pitchEst) > 30) {
        motorsArmed = false;
    }
